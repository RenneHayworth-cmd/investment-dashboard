...
def render_nav_chart(
    nav_df: pd.DataFrame,
    individual_df: pd.DataFrame | None = None,
) -> None:
    fig = go.Figure()
    fig.add_trace(
        go.Scatter(
...
def render_fund_rotation_mode() -> None:
    ...
    with st.form("fund_rotation_data_form"):
        if data_source == "上传文件":
            ...
            max_workers = st.number_input(
                "并发数", min_value=1, max_value=12, value=8, step=1
            )
    ...
    with st.sidebar:
        ...
        frequency_label = st.selectbox(
            "轮动频率", options=["每周一", "每月1号"], index=0
        )
        lookback_period = st.number_input(
            "动量周期", min_value=1, max_value=500, value=22, step=1
        )
        num_positions = st.number_input(
            "持仓数量", min_value=1, max_value=20, value=1, step=1
        )
    ...
    try:
        ...
        if data_source == "上传文件":
            for uploaded_file in uploaded_files:
                try:
                    raw_df = read_uploaded_table(
                        uploaded_file.getvalue(), uploaded_file.name
                    )
                    funds.append(
                        normalize_rotation_dataframe(
                            raw_df, fallback_name=uploaded_file.name
                        )
                    )
                except Exception as exc:
            ...
            if data_source == "场外基金":
                codes = [
                    item.strip()
                    for item in eastmoney_codes.replace(",", " ")
                    .replace("，", " ")
                    .replace("\n", " ")
                    .split()
                    if item.strip()
                ...
                for code in codes:
                    try:
                        ...
                        fund = normalize_rotation_dataframe(
                            raw_df, fallback_name=f"{code} 场外基金"
                        )
                ...
                codes = [
                    item.strip()
                    for item in tickflow_codes.replace(",", " ")
                    .replace("，", " ")
                    .replace("\n", " ")
                    .split()
                    if item.strip()
                ...
                for code in codes:
                    try:
                        ...
                        cached_df, cache_meta, _cache_period = load_rotation_cache(
                            cache_symbol
                        )
                        ...
                        funds.append(
                            normalize_rotation_dataframe(
                                raw_df, fallback_name=f"{symbol} {adjust_option}"
                            )
                        )
                    except Exception as exc:
                        errors.append(f"{code}: {exc}")
