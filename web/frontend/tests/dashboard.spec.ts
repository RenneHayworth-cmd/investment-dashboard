...
test('fixture 指数同卡显示正式与预览及上下轨',async({page})=>{
 ...
 await expect(card.locator('.price strong')).toHaveText('123.456')
 ...
 await expect(card.getByText('123.000',{exact:true})).toBeVisible()
 await expect(card.getByText('117.000',{exact:true})).toBeVisible()
