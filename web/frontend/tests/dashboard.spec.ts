...
test('无鉴权入口被拒绝',async ({ baseURL })=>{
 ...
 await expect(index.locator('details')).not.toHaveAttribute('open','')
 await expect(index.locator('.param')).toBeVisible()
 await expect(index.locator('.metrics')).toContainText('均线')
 const name=await index.locator('.index-title strong').boundingBox(), code=await index.locator('.code').boundingBox()
 expect(name!.y+name!.height).toBeLessThanOrEqual(code!.y)
 await index.locator('summary').click()
 await expect(index.locator('details dl').first()).toBeVisible()
 await expect(index.getByText('策略上轨',{exact:true})).toBeVisible()
 await expect(index.locator('.signal')).toContainText('正式状态')
 await expect(index.locator('.price strong')).toBeVisible()
 await page.getByRole('button',{name:'策略',exact:true}).click()
 await expect(page.getByText('模拟策略当前仓位',{exact:true})).toHaveCount(0)
 ...
 await expect(tip.locator('[data-field="净值"]')).toHaveText(row['净值'].toFixed(4))
...
test('fixture 指引全显示与最新记录展开',async({page})=>{
 const fixture=fixtureData()
 fixture.guidance=Array.from({length:7},(_,i)=>({日期:'2026-09-11',代码:`10000${i}`,基金名称:`指引${i}`,操作:'买入'}))
 fixture.strategy.trades=[{日期:'2026-09-10',代码:'159501',数量:100},{日期:'2026-09-11',代码:'159501',数量:200},{日期:'2026-09-11',代码:'510500',数量:300}]
 await page.route('**/api/dashboard',r=>r.fulfill({json:fixture}))
 await page.goto('/')
 await expect(page.getByTestId('guidance').locator('.record')).toHaveCount(7)
 await expect(page.getByTestId('guidance').getByText('指引6',{exact:true})).toBeVisible()
 await page.getByRole('button',{name:'策略',exact:true}).click()
 for(const name of ['总市值','可用资金'])await expect(page.locator('.strategy-summary').getByText(name,{exact:true})).toBeVisible()
 const trades=page.getByTestId('strategy-trades')
 await expect(trades.locator(':scope > .record-grid > .record')).toHaveCount(2)
 await expect(trades.locator('details')).not.toHaveAttribute('open','')
 await expect(trades.locator('details .record')).not.toBeVisible()
 const daily=page.getByTestId('strategy-daily')
 await expect(daily.locator(':scope > .record-grid > .record')).toHaveCount(1)
 await expect(daily.locator('details')).not.toHaveAttribute('open','')
 await page.getByRole('button',{name:'衍生品',exact:true}).click()
 await expect(page.locator('main details')).toHaveCount(0)
 await noOverflow(page)
})

test('fixture 指数同卡显示正式与预览及上下轨',async({page})=>{
 const fixture=fixtureData(), formal=fixture.index_formal[0]
 formal['择时判断']='空仓'
 formal['数据截止日']='2026-09-10'
 fixture.index_preview=[{...formal,最新收盘:123.456,对应均线:120.,'偏离率(%)':2.88,'当日涨跌幅(%)':1.5,择时判断:'买入',策略上轨:123.,策略下轨:117.,数据状态:'实时预判 14:52:00'}]
 await page.route('**/api/dashboard',r=>r.fulfill({json:fixture}))
 await page.goto('/')
 await page.getByRole('button',{name:'ETF',exact:true}).click()
 const card=page.locator('.index-reference').first()
 await expect(card.locator('.price strong')).toHaveText('123.46')
 await expect(card.locator('.metrics b').first()).toHaveText('120.00')
 await expect(card.locator('.signal')).toContainText('正式状态 空仓')
 await expect(card.locator('.signal')).toContainText('预览 买入')
 await expect(card.locator('.muted.small')).toContainText(String(formal['数据截止日']))
 await card.locator('summary').click()
 await expect(card.getByText('123.00',{exact:true})).toBeVisible()
 await expect(card.getByText('117.00',{exact:true})).toBeVisible()
 await expect(page.getByRole('heading',{name:'指数实时预判'})).toHaveCount(0)
 const etf=page.locator('.etf-card').first()
 await etf.locator('summary').click()
 await expect(etf.getByText('策略上轨',{exact:true})).toBeVisible()
 await expect(etf.getByText('103.936',{exact:true})).toBeVisible()
 await noOverflow(page)
})
