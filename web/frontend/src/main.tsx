...
function App() {
 ...
 {tab==='策略'&&<><section className="panel"><h2>净值曲线</h2>{data.strategy.daily.length ? <Chart rows={data.strategy.daily}/> : <p className="empty">正式数据不足，暂不生成净值曲线</p>}<p className="section-note">初始资金 {fmt(data.strategy_parameters['初始资金'])} 元 · {String(data.strategy_parameters['开始日期']).slice(0,10)} 起 · 前复权正式收盘 · 同收盘成交 · {fmt(data.strategy_parameters['整手份数'],0)} 份整手 · 单边费率 {fmt(data.strategy_parameters['单边费率'],5)}。初始持有须先退出再买入，承接仓位按业务规则延迟激活。</p><details><summary>策略摘要与费用</summary><Fields row={data.strategy.summary}/></details></section><details className="panel"><summary>模拟策略当前仓位</summary><p className="section-note">按正式收盘估值，与真实账户持仓无关。</p><Rows rows={data.strategy.positions} empty="暂无可确认的模拟持仓"/></details><TradePreview data={data}/><details className="panel"><summary>模拟交易记录</summary><Rows rows={[...data.strategy.trades].reverse()} empty="暂无模拟成交" limit={15}/></details><details className="panel"><summary>每日收益与盈亏</summary><Rows rows={[...data.strategy.daily].reverse()} limit={10}/></details></>}
