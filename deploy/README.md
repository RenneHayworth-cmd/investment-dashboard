...
用户已明确授权微盘股暂时忽略上述四个缺口：有效数据达到 MA 所需数量时允许预判，
状态中显示暂忽略的实际缺口数。例外仅适用于 BK1158 的这四天，其他标的和缺口
继续检查；不补造日线，补齐后提示自动消失。
...
模拟持仓列表默认展示标的／市值、浮动盈亏（金额及收益率）、份额、成本／现价，
手机在列表内左滑查看当日盈亏与账户权重，标的列固定。成本／现价为3位小数。
盘中字段由纯估值 service 提供，权重以包含现金的估算策略总资产为分母；
任一持仓缺报价时不返回实时权重，缺报价标的的估值字段为空。
正式列表复用正式持仓估值明细，保留当天已清仓标的的当日盈亏；
浮动盈亏不代表包含已实现收益的历史累计盈亏。所有盘中字段均不落盘。

ETF与指数卡片折叠区展示策略上下轨：MA × (1 ± 阈值)。计算复用
`calculate_etf_timing_snapshot`；盘中使用瞬时预览均线，绝不持久化。
收盘后仅将经过确认的正式日线写入既有缓存，上下轨由正式缓存重建，
不新增第二份指标数据库。512890无独立均线轨道。
