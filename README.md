...
循环后台更新（默认每 60 分钟检查一次，仅在 09:30-15:00 刷新）：
...
也可以在 Streamlit 左侧进入「任务与数据」页面，点击「立即运行一次」「启动后台循环」或「停止后台循环」。循环后台更新会在 09:30-15:00 之间按设定间隔刷新，其余时间跳过本轮更新。任务记录会写入 `cache.db` 的 `jobs` 表，后台循环 PID 会写入 `output/background_updater.pid`。

## 已网站化的桌面脚本

- 「指数监控」：展示主要指数、恒生科技、日经 225、韩国 KOSPI 和铁矿石主连的最新摘要、固定四列仪表盘卡片，以及按 MA20 偏离率排序的汇总表。首次打开优先展示本地缓存，今日缓存过期时后台自动触发更新。
